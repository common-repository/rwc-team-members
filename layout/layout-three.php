<?php
/**
 * Layout Three - Grid View with Left Popup for Member Details
 *
 * This file contains the template for displaying the team members in a grid view
 * on the frontend using layout three. In this layout, team members are presented in
 * a grid format, and their details appear in a left popup if enabled from the admin options.
 *
 * @package RWC Team Members - v0.5 - 25 July, 2024
 */
if ( ! defined( 'ABSPATH' ) ) exit; // Exit if accessed directly

if (!function_exists('rwctm_layout_three')) {
	function rwctm_layout_three( $t_name, $teamID, $t_activity, $t_combo, $tot_act, $t_lists, $m_options, $m_count, $flag ) {
		$i = 1; $j = 0;
		...
		if(isset($t_combo['popdir'])) {
			$disable_popup = $t_combo['popdir'];
		}
		...
		if(!empty($t_lists) && $t_combo['enable'] == 'yes' && $flag == 1) { ?>
			<div id="<?php echo esc_attr($teamID); ?>" class="rwctm-container<?php if($disable_popup != 'yes') { ?> rwctm-popup-corner<?php } ?>"><?php
				...
				<div class="rwctm-row" data-id="<?php echo esc_attr($max_cold); ?>"><?php
					foreach($m_options as $key => $value) :
						...
						<div class="rwctm-member member-<?php echo esc_html($memberValue['mid']); ?><?php echo ' '.esc_attr($layout_style); ?><?php if($disable_popup != 'yes') { ?> rwctm-member-corner<?php } ?>" data-category="<?php echo esc_attr($category); ?>" data-mid="<?php echo esc_attr($dataid); ?>">
							...
							<?php /* Popup/Modal */ ?>
							<?php if($t_combo['layout'] == 'popup') { ?>
								<?php if($disable_popup != 'yes') { ?>
									<div class="rwctm-container <?php echo esc_attr($teamID); ?>">
										<div id="member-<?php echo esc_html($memberValue['mid']); ?>" class="rwctm-modal-corner rwc-info-group">
											<div class="rwctm-overlay"></div>
											<div class="rwctm-modal-content member-<?php echo esc_html($memberValue['mid']); ?>">
